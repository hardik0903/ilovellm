  ...
  const handleChat = async (e) => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:8000/api/vector/query', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ query: userMessage, n_results: 10, document_id: documentId })
      });
      const data = await res.json();
      
      if (data.success && data.results.documents[0].length > 0) {
         const matchingChunks = data.results.documents[0];
         const metadata = data.results.metadatas[0];
         
         if (matchingChunks.length > 0) {
            // Build Context with citations
            const contextLines = matchingChunks.map((chunk, i) => {
              const meta = metadata[i] || {};
              const pageStr = meta.page ? ` [Page ${meta.page}]` : "";
              return `Chunk ${i + 1}${pageStr}:\n${chunk}`;
            });
